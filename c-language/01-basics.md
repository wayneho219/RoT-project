...
C 裡面同一個數字可以用三種方式寫，存進記憶體的值完全一樣：

```c
255          // 沒有前綴 → 十進位
0xFF         // 0x 前綴  → 十六進位
0b11111111   // 0b 前綴  → 二進位
0377         // 0 前綴   → 八進位（幾乎不用）

// 以上四個值完全相同，只是寫法不同
```

嵌入式幾乎都用十六進位（`0x`），因為硬體暫存器的 bit 用十六進位看最直覺。
...
型別名稱解讀：
- `u` = unsigned（無符號，不能是負數）；沒有 `u` = signed（可以是負數）
- 數字 = bits 數（8 / 16 / 32 / 64）
- `_t` = type（慣例後綴）
...
└──────────┴──────────┴──────────┴──────────┘
 範圍：0 ~ 4,294,967,295（0xFFFFFFFF）
```

> [!important] 嵌入式規則
> 嵌入式程式碼只用 `uintN_t` / `intN_t`，不用裸的 `int`
...
記憶體（從位址 0x1000 開始）：
       0x1000     0x1001     0x1002     0x1003     0x1004     ...    0x1007
      ┌──────────┬──────────┬──────────┬──────────┬──────────┬──────┬──────────┐
      │    10    │    00    │    00    │    00    │    20    │  00  │    00    │
      └──────────┴──────────┴──────────┴──────────┴──────────┴──────┴──────────┘
      │◄──────────── p.x (uint32_t, 4 bytes) ────────►│◄── p.y (uint32_t, 4 bytes) ──►│
```
...
stateDiagram-v2
    [*] --> IDLE
    IDLE --> READING : boot_next()
    READING --> VERIFYING : boot_next()
    VERIFYING --> BOOTING : boot_next()
    BOOTING --> DONE : boot_next()
    READING --> ERROR : boot_fail()
    VERIFYING --> ERROR : boot_fail()
    BOOTING --> ERROR : boot_fail()
    DONE --> [*]
```
...
**Little-Endian 記憶體佈局示意：**

```
uint32_t value = 0x00018000;

記憶體（低位址在左）：
位址：   [addr+0]  [addr+1]  [addr+2]  [addr+3]
值：     [  0x00 ] [  0x80 ] [  0x01 ] [  0x00 ]
          ↑ 最低位                       ↑ 最高位

組合回去：0x00 | (0x80 << 8) | (0x01 << 16) | (0x00 << 24)
        = 0x00018000 ✓
```
...
> [!tip] 複習問題
> 1. `uint8_t` 和裸 `int` 在嵌入式的差異？為什麼暫存器操作要用 `uint32_t` 而非 `int32_t`？
> 2. `static` 在檔案層級與函式內部各有什麼語意？對應 C# 的什麼概念？
> 3. Little-endian 下，`uint32_t value = 0x00018000` 存到 4 個 byte 的順序（從低位址到高位址）是什麼？

→ [[02-pointers|Module 2：指標與記憶體]]
